...
function sendWebhook(channel, msg, files) {
  channel.fetchWebhooks().then(() => { // get webhooks
    let webhook = false
    ...
    if (!webhook) { //create one if not found
      channel.createWebhook("mvbot").then((webhook) => {
        return webhook.send(msg.content, { // send with new webhook
      ...
      return webhook.send(msg.content, { // send with existing hook
        username: msg.author.username,
        avatar_url: msg.author.avatarURL('png', true),
        files: files
      })
